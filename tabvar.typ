  ...
  context {
    ...
    // debug:true,
    {
      ...
      for i in range(longeur) {
          content(("l", "-|", values.at(i).at(0)),values.at(i).at(1), name:"depart"+str(i),padding:.25)

          content(values.at(i).at(0),values.at(i).at(2),name:"fin"+str(i),frame:"rect",fill:white,stroke:none,padding:(y:.05))
          ...
          } else {if values.at(i).at(3) == "l" { line("depart"+str(i)+".south","fin"+str(i)+".north",stroke:(thickness:.6pt,paint:coulfleche),..pointilles)}
          }
          }
...
#tabvar(
  ...
  values:(("a0.20%",$beta$,$0$,"l"),("a1.50%",$5$,$0$,"f"),teal),
)
